...
import { DELIVERY_SLOTS, formatIQD, linePrice, productName } from '../data/mock'
...
export default function CheckoutScreen({ navigation }) {
  ...
  const [slot, setSlot] = useState(DELIVERY_SLOTS[2])
  ...
  const walletOk = walletBalance >= cartPayable
  ...
  const canSubmit = () => {
    if (isOffline || belowMin || payState === 'confirming') return false
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={[]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12, paddingBottom: 28 }}>
        ...
        <Option on={when === 'now'} onPress={() => setWhen('now')} isRTL={isRTL} title={t('now')} hint={`${cartStore.eta} ${t('min')}`} />
        <Option on={when === 'later'} onPress={() => setWhen('later')} isRTL={isRTL} title={t('schedule')} hint={t('pickSlot')} />
        {when === 'later' ? (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingVertical: 2 }}>
            {DELIVERY_SLOTS.map((s) => {
              ...
              return (
                ...
                >
                  <Text style={{ fontWeight: '800', color: active ? '#fff' : colors.ink }}>{s}</Text>
                </SoftPress>
              )
            })}
          </ScrollView>
        ...
        >
          ...
          <SummaryRow label={t('total')} value={formatIQD(cartPayable, lang)} isRTL={isRTL} bold />
        </View>
        ...
        >
          <Text style={{ color: '#fff', fontWeight: '800', fontSize: 15 }}>
            {payState === 'confirming'
              ? t('paymentConfirming')
              : `${t('confirmOrder')} · ${formatIQD(cartPayable, lang)}`}
          </Text>
        </SoftPress>
      </ScrollView>
    </SafeAreaView>
