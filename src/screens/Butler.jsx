...
export default function ButlerScreen({ navigation }) {
  ...
  const initialDropoff = user?.address?.area
    ? `${user.address.area}${user.address?.details ? `، ${user.address.details}` : ''}`
    : ''
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={[]}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 12 }}>
        ...
        <Pressable
          onPress={() => {
            if (!need.trim() || !pickup.trim() || !dropoff.trim()) {
              setError(t('butlerFillRequired'))
              return
            }
            setError('')
            const job = requestButler({ need: need.trim(), pickup: pickup.trim(), dropoff: dropoff.trim(), when })
            setNeed('')
            setPickup('')
