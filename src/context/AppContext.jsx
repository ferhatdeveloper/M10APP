...
export function AppProvider({ children }) {
  ...
  const [stockAlerts, setStockAlerts] = useState([]) // [{ storeId, productId, createdAt }]
  const [productReviews, setProductReviews] = useState([]) // user-submitted reviews
  const [courierChats, setCourierChats] = useState({}) // { orderId: [{ from, text, ts }] }
  ...
  const getCourierChat = (orderId) => courierChats[orderId] || []
  const sendCourierMessage = (orderId, text) => {
    const clean = String(text || '').trim()
    if (!clean) return
    setCourierChats((prev) => {
      const cur = prev[orderId] || []
      const next = { ...prev, [orderId]: [...cur, { from: 'me', text: clean, ts: Date.now() }] }
      persist('m10-courier-chats', next)
      return next
    })
    // Simulate a courier reply after a short delay using canned responses
    setTimeout(() => {
      const canned = [
        'Anlaşıldı, çok teşekkürler.',
        'Tamam, hemen iletiyorum.',
        'Bir dakika kontrol edeyim.',
        'Yola çıktım, 5 dakikada oradayım.',
      ]
      ...
      setCourierChats((prev) => {
        ...
        const next = {
          ...prev,
          [orderId]: [...cur, { from: 'courier', text: reply, ts: Date.now() }],
        }
        persist('m10-courier-chats', next)
        return next
      })
  ...
  const getProductReviews = (productId) => {
    const user = productReviews.filter((r) => r.productId === productId)
    const seed = seedProductReviews
      .filter((r) => r.productId === productId)
      .map((r) => ({ ...r, seed: true, createdAt: Date.now() - r.days * 86400_000 }))
    return [...user, ...seed].sort((a, b) => b.createdAt - a.createdAt)
  }

  const addReview = (productId, { rating, text, userName }) => {
    const cleanText = String(text || '').trim()
    if (!cleanText) return false
    const r = {
      productId,
      user: userName || user?.name || 'Misafir',
    ...
    setProductReviews((prev) => {
      const next = [r, ...prev]
      persist('m10-product-reviews', next)
      return next
    })
    return true
  }

  const isOnStockAlert = (storeId, productId) =>
    stockAlerts.some((a) => a.storeId === storeId && a.productId === productId)

  const toggleStockAlert = (storeId, productId) => {
    setStockAlerts((prev) => {
      const idx = prev.findIndex((a) => a.storeId === storeId && a.productId === productId)
      let next
      if (idx >= 0) {
        next = prev.filter((_, i) => i !== idx)
      } else {
        next = [...prev, { storeId, productId, createdAt: Date.now() }]
      }
      persist('m10-stock-alerts', next)
      return next
    })
  }
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const storedStockAlerts = await load('m10-stock-alerts', [])
      if (Array.isArray(storedStockAlerts)) setStockAlerts(storedStockAlerts)
      const storedReviews = await load('m10-product-reviews', [])
      if (Array.isArray(storedReviews)) setProductReviews(storedReviews)
      const storedChats = await load('m10-courier-chats', {})
      if (storedChats && typeof storedChats === 'object') setCourierChats(storedChats)
  ...
  const setQty = (productId, qty, variantId = null) => {
    setCart((prev) => {
      ...
      const oKey = line ? `${resolveStoreId(line.storeId)}:${productId}` : null
      const liveOverrideStock = line ? storeOverrides[oKey]?.stock : null
      const stock = liveOverrideStock != null ? liveOverrideStock : product?.stock ?? 99
  ...
  const reorder = (order) => {
    ...
    const next = order.items
      .map((i) => {
        ...
        if (!p) return null
        const oKey = `${storeId}:${i.productId}`
        const liveStock = storeOverrides[oKey]?.stock != null ? storeOverrides[oKey].stock : p.stock ?? 99
        if (liveStock <= 0) return null
        return { storeId, productId: i.productId, qty: Math.min(i.qty, liveStock) }
      })
      .filter(Boolean)
  ...
  const addListToCart = (list) => {
    ...
    if (cart.length && cart[0].storeId !== storeId) {
      return { ok: false, added: 0, reason: 'otherStore' }
    }
    let added = 0
  ...
  const loginWithProvider = (provider) => {
    // Mock social login — generate a stable demo phone from the provider name.
    const mockPhones = {
      google: '0770 999 1111',
      apple: '0770 999 2222',
    }
    const mockNames = {
      google: 'Google User',
    ...
    const phone = mockPhones[provider] || mockPhones.google
    const name = mockNames[provider] || 'Demo User'
    return login(phone, name)
  }
